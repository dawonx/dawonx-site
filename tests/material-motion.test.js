...
const geometry = require('../material-geometry');
...
function fixture(options = {}) {
    ...
    const records = { contexts: 0, draws: [], uploads: 0, generations: 0, textureCalls: 0, uniforms: {}, pending: new Map() };
    const gl = {};
    for (const name of ['VERTEX_SHADER', 'FRAGMENT_SHADER', 'LINK_STATUS', 'ARRAY_BUFFER', 'ELEMENT_ARRAY_BUFFER', 'STATIC_DRAW', 'FLOAT', 'TEXTURE0', 'TEXTURE_2D', 'UNPACK_FLIP_Y_WEBGL', 'TEXTURE_WRAP_S', 'TEXTURE_WRAP_T', 'CLAMP_TO_EDGE', 'TEXTURE_MIN_FILTER', 'TEXTURE_MAG_FILTER', 'LINEAR', 'RGBA', 'UNSIGNED_BYTE', 'LINES', 'UNSIGNED_SHORT', 'DEPTH_TEST', 'BLEND', 'SRC_ALPHA', 'ONE_MINUS_SRC_ALPHA', 'ONE', 'COLOR_BUFFER_BIT']) gl[name] = name;
    gl.NO_ERROR = 0;
    for (const name of ['deleteShader', 'deleteBuffer', 'deleteTexture', 'deleteProgram', 'shaderSource', 'compileShader', 'attachShader', 'bindAttribLocation', 'linkProgram', 'useProgram', 'bindBuffer', 'bufferData', 'enableVertexAttribArray', 'vertexAttribPointer', 'activeTexture', 'bindTexture', 'pixelStorei', 'texParameteri', 'viewport', 'clearColor', 'clear', 'enable', 'disable', 'blendFuncSeparate']) gl[name] = () => {};
    for (const name of ['createProgram', 'createShader', 'createBuffer']) gl[name] = () => ({});
    gl.getProgramParameter = () => !options.shaderFailure;
    gl.createTexture = gl.texImage2D = gl.bindTexture = () => { records.textureCalls++; throw new Error('No image sampling is allowed'); };
    gl.bufferData = (_, data) => { records.uploads++; assert(data.length > 1000); };
    gl.getError = () => options.bufferFailure ? 1285 : 0;
    gl.getAttribLocation = (_, name) => ({ a_position: 0, a_tangent: 1, a_material: 2 })[name];
    ...
    gl.drawArrays = (kind, start, count) => { assert.equal(kind, gl.LINES); assert(count > 1000); records.draws.push(records.uniforms.u_time); };
    const canvas = target({ getContext() { records.contexts++; return options.unsupported ? null : gl; } });
    const elements = { 'material-hero': hero, 'material-canvas': canvas, 'motion-toggle': button };
    ...
    const window = target({ innerHeight: 1000, innerWidth: options.width || 1440, devicePixelRatio: options.dpr || 2, matchMedia: () => reduced, IntersectionObserver, ResizeObserver });
    vm.runInNewContext(source, { window, document, navigator: { connection }, IntersectionObserver, ResizeObserver,
        MaterialGeometry: { ...geometry, createFibers: (...args) => { records.generations++; return geometry.createFibers(...args); } },
    ...
    return { hero, button, canvas, document, window, reduced, connection, records, step,
...
test('unsupported graphics, shader failure and buffer failure preserve the image fallback', () => {
    for (const options of [{ unsupported: true }, { shaderFailure: true }, { bufferFailure: true }]) {
        const f = fixture(options);
        assert.equal(f.records.pending.size, 0);
        assert.equal(f.button.hidden, true);
        assert(!Object.hasOwn(f.hero.dataset, 'motionReady'));
...
test('real fiber geometry renders without an image element or any texture API', () => {
    const f = fixture();
    assert.equal(f.records.contexts, 1);
    assert.equal(f.records.uploads, 1);
    assert(!Object.hasOwn(f.hero.dataset, 'motionReady'));
    f.step(0); f.step(34); f.step(68);
    assert(Object.hasOwn(f.hero.dataset, 'motionReady'));
    assert.equal(f.records.textureCalls, 0);
    assert.equal(f.records.generations, 1, 'Geometry is generated once, not every frame');
});
...
test('paused geometry is restored after context loss and preference changes', () => {
    const f = fixture();
    f.step(0); f.step(34); f.button.emit('click');
    const held = f.records.draws.at(-1);
...
test('portrait layouts use the current aspect ratio without image dimensions', () => {
    const f = fixture({ width: 390, height: 740 });
    assert.equal(f.records.uniforms.u_aspect, 390 / 740);
    f.step(0);
    assert(Object.hasOwn(f.hero.dataset, 'motionReady'));
    assert.equal(f.records.textureCalls, 0);
});
