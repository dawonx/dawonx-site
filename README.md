...
The live landing artwork is procedural 3D geometry. `material-geometry.js` generates seeded fiber paths around irregular pores and maps them onto a folded surface. Its vertex shader changes the positions, depth and orientation over time. `material-motion.js` renders these line vertices directly, without image loading, textures or an additional rendering library. It is generative artwork, not a calibrated material physics simulation. The earlier `assets/optimized/material-membrane-v1.webp` is used only as a static fallback when motion is disabled or WebGL is unavailable; it is hidden during live rendering. Rendering is capped at 30 fps and 1.5 million pixels, with fewer fibers on mobile. Pause/resume, reduced-motion and data-saving preferences, offscreen/hidden-page suspension, and WebGL context recovery keep navigation independent of the effect.
