...
Every page shares the same navigation and header dimensions. On the home page, the header stays transparent over the artwork, then smoothly gains a solid background as the final quarter of the hero passes beneath it; other pages use a solid sticky header. The hero follows the current viewport height, with room for its content on short screens. Published navigation links, stylesheets and scripts carry a release version derived from the build output, so navigation after an update requests fresh pages and matching assets instead of reusing old header markup.

Typography uses self-hosted D-DIN Regular and Bold, with an italic face for article emphasis and system fallbacks for unsupported characters. The original WOFF2 files, provenance, copyright notice and SIL Open Font License are bundled in `assets/vendor/d-din/`. Primary faces are preloaded with `font-display: swap`; no external font stylesheet is required.

The header and footer wordmark uses the site's D-DIN Regular font. It rests as DX; hover or keyboard focus smoothly reveals AWON and moves X outward. Its reserved width keeps navigation still, and reduced-motion preferences remove the transition. Touch users retain a direct home link.
...
The model follows the independent-wave and slope-detail principles in [NVIDIA GPU Gems, Chapter 1](https://developer.nvidia.com/gpugems/gpugems/part-i-natural-effects/chapter-1-effective-water-simulation-physical-models), the dispersion relation in [Tessendorf's ocean course notes](https://jtessen.people.clemson.edu/reports/papers_files/coursenotes2004.pdf), and the [Schlick reflection approximation](https://developer.nvidia.com/gpugems/gpugems3/part-iii-rendering/chapter-17-robust-multiple-specular-reflections-and-refractions). The shader is authored for this site. A mild artistic domain warp curves the wave fronts, with chain-rule derivatives keeping normals aligned; attenuated small waves soften reflection edges. Highlights favour the right side. Water rendering is capped at 30 fps and 520,000 pixels (240,000 on mobile); unsupported graphics fall back to the gradients. Two offset samples of the cached grain tile crossfade over 1.8 seconds in each direction inside one soft-light layer. Only opacity animates; the texture stays in place, its maximum strength stays at 0.51, and the dissolve gently softens it between samples. `ambient-motion.js` shares visibility, reduced-motion and data-saving state with all animated effects. Hidden/offscreen pages pause motion, reduced motion freezes the background, and context restoration resumes without a time jump. The gradients and grain remain visible without JavaScript, and the landing has no motion button. Earlier fiber artwork remains in Git history and its saved image is no longer loaded by the hero.
