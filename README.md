...
The landing background blends three soft CSS gradients in teal, blue and green. `water-surface.js` adds a transparent WebGL layer of crossing water waves and reflected glints, calculated from surface slopes without images, videos or rendering libraries. Highlights favour the right side and sit below the text shade. Water rendering is capped at 30 fps and 520,000 pixels (240,000 on mobile); unsupported graphics fall back to the gradients. `ambient-motion.js` shares the visibility, reduced-motion and data-saving state with both effects. Hidden/offscreen pages stop rendering, reduced motion freezes the background, and WebGL context restoration resumes without a time jump. The gradients remain visible without JavaScript, and the landing has no motion button. Earlier fiber artwork remains in Git history and its saved image is no longer loaded by the hero.
