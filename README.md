...
Every page shares the same navigation and header dimensions. On the home page, the header overlays the full-viewport artwork and gains a solid background after scrolling; other pages use a solid sticky header. The hero follows the current viewport height, with room for its content on short screens. Published navigation links, stylesheets and scripts carry a release version derived from the build output, so navigation after an update requests fresh pages and matching assets instead of reusing old header markup.
