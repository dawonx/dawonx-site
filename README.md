...
The landing background blends three soft CSS gradients in teal, blue and green. Only transforms and opacity animate; the background loads no images, videos or rendering libraries. `ambient-motion.js` varies the timing once per visit and suspends motion offscreen, in hidden pages and when data saving is enabled. Reduced-motion preferences disable animation, and the gradients remain visible without JavaScript. The landing has no motion button. Earlier fiber artwork remains in Git history and its saved image is no longer loaded by the hero.
